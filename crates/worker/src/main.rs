...
use worker::ingestion::{app, debezium};

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    dotenv::dotenv().ok();

    let log_file = rolling::daily("logs", "worker.log"); // Лог-файл, создается каждый день
    ...
    let debezium_task = tokio::spawn(async move {
        if let Err(e) = debezium::run_loop(pool, brokers, group, topic).await {
            error!(error=%e,"Error in debezium task!");
        }
    });

    info!("Starting processing tasks...");

    let (r_app, r_debezium) = tokio::join!(app_task, debezium_task);
    if let Err(e) = r_app {
        error!(error=%e, "Spawn App");
    }

    if let Err(e) = r_debezium {
        error!(error=%e,"Spawn Debezium");
    }
